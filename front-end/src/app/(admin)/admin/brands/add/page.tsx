...
export default function AddBrandPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await dispatch(createBrand({ name, image })).unwrap();
            toast.success("تم إضافة الماركة بنجاح");
            router.push("/admin/brands");
        } catch (err: any) {
            toast.error(err || "فشل إضافة الماركة");
        }
    };

    return (
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-sm border border-gray-100" style={{ fontFamily: "var(--font-cairo)" }}>
            <h1 className="text-2xl font-bold mb-6" style={{ color: "var(--black-color)" }}>إضافة ماركة جديدة</h1>
