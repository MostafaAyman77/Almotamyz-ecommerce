...
export default function AddSubCategoryPage() {
    ...
    const { loading } = useSelector((state: RootState) => state.subcategory);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await dispatch(createSubCategory({ name, category })).unwrap();
            toast.success("تم إضافة التصنيف الفرعي بنجاح");
            router.push("/admin/subcategories");
        } catch (err: any) {
            toast.error(err || "فشل إضافة التصنيف الفرعي");
        }
    };

    return (
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-sm border border-gray-100" style={{ fontFamily: "var(--font-cairo)" }}>
            <h1 className="text-2xl font-bold mb-6" style={{ color: "var(--black-color)" }}>إضافة تصنيف فرعي جديد</h1>
